...
export default function Login() {
  ...
  const schema = z.object({
    ...
    password: z.string().min(1, errorCopy.passwordRequired),
  });
    ...
    <div className="min-h-screen relative overflow-hidden flex items-center justify-center bg-slate-50 text-slate-900 dark:bg-slate-900 dark:text-slate-100">
      ...
      <div className="container mx-auto px-4 md:px-6 relative z-10 flex justify-center">
        <div className="w-full max-w-md mx-4 my-12">
          <div className="rounded-3xl p-10 sm:p-12 border border-slate-200/80 bg-white/95 shadow-[0_20px_60px_rgba(15,23,42,0.12)] backdrop-blur dark:border-white/10 dark:bg-gradient-to-b dark:from-slate-900/90 dark:via-slate-950 dark:to-slate-950 dark:shadow-[0_24px_80px_rgba(15,23,42,0.9)]">
            <div className="text-center mb-6">
              <h1 className="text-3xl font-bold text-slate-900 dark:text-white mb-2">
                {loginCopy.title || t("auth.login.title")}
              </h1>
              <p className="text-sm text-slate-500 dark:text-slate-400">
                {loginCopy.subtitle || "เข้าสู่ระบบเพื่อจัดการโปรไฟล์และติดตามรายงานของคุณ"}
              </p>
            </div>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
              ...
              <Link
                to="/register"
                className="w-full h-12 rounded-xl font-semibold border border-slate-300 text-slate-800 hover:border-sky-500 hover:bg-sky-50 flex items-center justify-center gap-2 transition-all dark:border-slate-600 dark:text-white dark:hover:border-sky-400 dark:hover:bg-slate-900/60"
              >
